...
DECLARE
	...
	v_cd_ref bigint;
	v_date_min timestamp;
	v_date_max timestamp;
	v_doy_min integer;
	v_doy_max integer;
...
BEGIN
    ...
    BEGIN
		...
		SELECT 
		    taxonomie.find_cdref_es(cd_nom),
		    date_min,
		    date_max
		INTO
		    v_cd_ref,
		...
		v_doy_min := extract(doy from v_date_min);
		v_doy_max := extract(doy from v_date_max);
		...
		EXECUTE format($sql$
		    ...
		    LEFT JOIN LATERAL (
		        SELECT
		            ...
		            bool_or(
		                $4 >= rt.date_obs_favorable_inf
		            AND $5 <= rt.date_obs_favorable_sup
		            ) AS fully_inside_any,
		            -- au moins une période qui chevauche l’intervalle obs
		            bool_or(
		                $5 >= rt.date_obs_favorable_inf
		            AND $4 <= rt.date_obs_favorable_sup
		            ) AS overlaps_any
		        FROM %I.%I rt
		        WHERE rt.cd_ref = $6
